...
describe('HDFilme', () => {
  ...
  test('handle movie stream page with data-link and iframe hosters', async () => {
    const searchDoc = '<div><a href="/stream/avatar-2009.html" title="Avatar 2009">Avatar</a></div>';
    const streamPageDoc = '<div><div data-link="//voe.sx/e/avatar"></div><div data-link="streamtape.com/e/123"></div><iframe src="https://streamtape.com/e/123"></iframe><a href="javascript:void(0)">Ignore</a></div>';
    ...
    (fetcher.text as jest.Mock)
      ...
      .mockResolvedValueOnce(streamPageDoc);
  ...
  test('handle movie stream with year match in search', async () => {
    const searchDoc = '<div><a href="/stream/dune-2021.html" title="Dune 2021">Dune</a></div>';
    const streamPageDoc = '<div><div data-link="https://voe.sx/e/dune"></div></div>';
  ...
  test('handle no candidates found', async () => {
    const noResultsData = '<div>No results</div>';
    (fetcher.json as jest.Mock).mockResolvedValueOnce({ title: 'Unknown Movie', release_date: '2024-01-01' });
    (fetcher.text as jest.Mock).mockResolvedValueOnce(noResultsData);
  ...
  test('handle empty stream page content gracefully', async () => {
    const searchDoc = '<div><a href="/stream/empty.html">Empty</a></div>';
    const emptyPageDoc = '<div></div>';
    (fetcher.json as jest.Mock).mockResolvedValueOnce({ title: 'Empty Movie', release_date: '2024-01-01' });
    (fetcher.text as jest.Mock)
      .mockResolvedValueOnce(searchDoc)
      .mockResolvedValueOnce(emptyPageDoc);
